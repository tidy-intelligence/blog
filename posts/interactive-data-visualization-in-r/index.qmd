---
title: "Interactive Data Visualization in R: ggiraph vs plotly vs highcharter"
description: "A comparison of dynamic visualization packages in R."
metadata:
  pagetitle: "Interactive Data Visualization in R: ggiraph vs plotly vs highcharter"
...
for(j in seq_along(species_unique)) {
  ...
  fig_full <- fig_full |>  
    hc_add_series(data = list_parse2(line_data), type = "line", marker = "none",
                  color = highchart2()$hc_colors[j])
}
...
I personally like the visual appearance and user experience of `highcharter`, but I still prefer the flexibility of `ggplot2` in combination with `plotly`. The `ggplotly()` function provides a powerful tool for quick, interactive data visualization of complex relationships that you prototype with `ggplot2`. 
