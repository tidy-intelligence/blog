...
date: "2023-05-22"
description: A simple implementation for prototyping multiple collaborative filtering algorithms using R
image: thumbnail.png
...
For this post, we rely on the `tidyverse` family of packages, `scales` for scale functions for visualization, and `recommenderlab`[^2] - a package that provides an infrastructure to develop and test collaborative filtering recommender algorithms.  
...
```{r}
#| fig-alt: "ROC curves of stock recommender algorithms. The figure shows that the association rules mining algorithm achieves the highest true positive rate for any false positive rate."
results_tbl |>
...
```{r}
#| fig-alt: "Precision-Recall curves of stock recommender algorithms. The figure shows that the association rules mining algorithm achieves the highest precision for any level of recall."
results_tbl |>
